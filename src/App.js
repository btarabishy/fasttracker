...
export default function App(){
    ...
    navigator.geolocation.getCurrentPosition(async pos=>{
      try{
        ...
        if(data.code===200){
          ...
          // parse fajr and maghrib into today's Date objects
          ...
          flash(`📍 Prayer times loaded for your location`);
        }else{throw new Error("API error");}
      }catch(e){setPrayer(p=>({...p,loading:false,error:"Could not load prayer times"}));flash("⚠️ Using approximate prayer times");}
    },err=>{setPrayer(p=>({...p,loading:false,error:"Location denied"}));flash("⚠️ Location access denied — using approximate times");});
  },[]);

  // when mode becomes religious, fetch times
  useEffect(()=>{
    if(mode===MODES.RELIGIOUS)fetchPrayerTimes();
  },[mode]);
  ...
  useEffect(()=>{
    ...
    autoRef.current=setInterval(()=>{
      ...
      if(now>=fajrMs&&now<maghribMs&&!fasting){
        setStartMs(fajrMs); setElapsed(Math.floor((now-fajrMs)/1000)); setFasting(true);
        flash("🌙 Auto-fast started at Fajr");
      }
      if(now>=maghribMs&&fasting){
        endFastSilent(Math.floor((maghribMs-fajrMs)/1000));
        flash("🌙 Auto-fast ended at Maghrib — well done!");
      }
    },5000);
  ...
  function endFastSilent(hrs){setHistory(h=>[{id:Date.now(),date:new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}),hours:+(hrs/3600).toFixed(1),type:"Religious",done:true},...h]);setFasting(false);setElapsed(0);}
  function endFast(){if(!fasting)return;const hrs=+(elapsed/3600).toFixed(1);setHistory(h=>[{id:Date.now(),date:new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}),hours:hrs,type:mode===MODES.HEALTH?"Health":"Religious",done:hrs>=preset.fast},...h]);setFasting(false);setElapsed(0);flash(`Saved — ${hrs}h 📝`);}

  function switchMode(){setFasting(false);setElapsed(0);clearInterval(timerRef.current);setScreen("onboard");}
  ...
  const phoneStyle={width:390,minHeight:844,margin:"0 auto",background:"#09090F",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif",color:"#fff",position:"relative",overflow:"hidden",borderRadius:52,boxShadow:"0 40px 100px rgba(0,0,0,0.8)",display:"flex",flexDirection:"column"};

  if(screen==="onboard")return(<div style={phoneStyle}><StatusBar/><Onboard onSelect={m=>{setMode(m);setScreen("main");}}/></div>);
...
function StatusBar(){return(<div style={{padding:"14px 28px 0",display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:13,fontWeight:600,flexShrink:0}}><span>9:41</span><div style={{width:126,height:34,background:"#09090F",borderRadius:20,border:"1.5px solid #1e1e2a",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{width:68,height:5,background:"#1a1a2a",borderRadius:4}}/></div><span>●●● 🔋</span></div>);}
...
function Onboard({onSelect}){return(<div style={{flex:1,display:"flex",flexDirection:"column",padding:"32px 28px 40px",gap:24}}>
  <div style={{textAlign:"center",paddingTop:12}}><div style={{fontSize:52}}>⏱</div><h1 style={{margin:"10px 0 0",fontSize:28,fontWeight:800}}>Welcome to<br/>FastTracker</h1><p style={{margin:"10px 0 0",fontSize:15,color:"#888",lineHeight:1.5}}>Your personal fasting companion.<br/>Choose your journey to get started.</p></div>
  <div style={{display:"flex",flexDirection:"column",gap:14,marginTop:8}}>
    <OCard emoji="💪" title="Health" badge="Wellness" desc="Intermittent fasting for health & metabolic goals." accent="#60A5FA" onClick={()=>onSelect(MODES.HEALTH)}/>
    <OCard emoji="🌙" title="Religious" badge="Ramadan" desc="Auto prayer times based on your GPS location." accent="#A78BFA" onClick={()=>onSelect(MODES.RELIGIOUS)}/>
  </div>
...
function OCard({emoji,title,badge,desc,accent,onClick}){const[h,sH]=useState(false);return(<button onClick={onClick} onMouseEnter={()=>sH(true)} onMouseLeave={()=>sH(false)} style={{background:h?`${accent}18`:"rgba(255,255,255,0.04)",border:`1.5px solid ${h?accent+"55":"rgba(255,255,255,0.09)"}`,borderRadius:20,padding:"20px 22px",textAlign:"left",cursor:"pointer",color:"#fff",transition:"all 0.2s",display:"flex",alignItems:"center",gap:14,width:"100%"}}><span style={{fontSize:30}}>{emoji}</span><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}><span style={{fontSize:18,fontWeight:700}}>{title}</span><span style={{fontSize:11,fontWeight:600,color:accent,background:`${accent}20`,borderRadius:8,padding:"2px 8px"}}>{badge}</span></div><div style={{fontSize:13,color:"#777",lineHeight:1.4}}>{desc}</div></div><span style={{color:accent,fontSize:22}}>›</span></button>);}
...
function Home({mode,fasting,elapsed,remaining,progress,preset,setPreset,stage,ringColor,R,CIR,drawn,startFast,endFast,prayer,prayerFajr,prayerMaghrib,fmt,fmtS,toHHMM,switchMode,autoFast,setAutoFast,fetchPrayerTimes}){
  ...
  const now=new Date();
  const isR=mode===MODES.RELIGIOUS;
  return(<div style={{padding:"18px 24px 0"}}>
    {/* header with mode-switch button */}
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
      <div>
        <h2 style={{margin:0,fontSize:22,fontWeight:800}}>FastTracker</h2>
        <p style={{margin:"3px 0 0",fontSize:13,color:"#555"}}>{now.toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})}</p>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:6,alignItems:"flex-end"}}>
        ...
        {/* quick switch button */}
        <button onClick={switchMode} style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:"4px 10px",fontSize:11,color:"#888",cursor:"pointer",fontWeight:600}}>⇄ Switch Mode</button>
      </div>
    </div>
    ...
    {isR&&prayer.error&&<div style={{background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:12,padding:"10px 14px",marginBottom:12,fontSize:13,color:"#EF4444",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span>⚠️ {prayer.error}</span><button onClick={fetchPrayerTimes} style={{background:"rgba(239,68,68,0.2)",border:"none",borderRadius:8,padding:"4px 10px",color:"#EF4444",cursor:"pointer",fontSize:12,fontWeight:600}}>Retry</button></div>}
    {isR&&prayer.city&&!prayer.loading&&!prayer.error&&<div style={{background:"rgba(167,139,250,0.08)",border:"1px solid rgba(167,139,250,0.15)",borderRadius:12,padding:"8px 14px",marginBottom:12,fontSize:12,color:"#A78BFA",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span>📍 {prayer.city}</span><button onClick={fetchPrayerTimes} style={{background:"none",border:"none",color:"#666",cursor:"pointer",fontSize:11}}>Refresh</button></div>}
    ...
    {isR&&<div style={{background:"rgba(167,139,250,0.08)",border:"1px solid rgba(167,139,250,0.2)",borderRadius:14,padding:"14px 18px",marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        <div style={{textAlign:"center"}}><div style={{fontSize:22}}>🌅</div><div style={{fontSize:11,color:"#666"}}>Fajr</div><div style={{fontSize:13,fontWeight:700}}>{toHHMM(prayerFajr)}</div></div>
        <div style={{flex:1,height:4,background:"rgba(167,139,250,0.2)",borderRadius:2,overflow:"hidden",margin:"0 14px"}}><div style={{width:`${progress*100}%`,height:"100%",background:"linear-gradient(90deg,#7C3AED,#A78BFA)",transition:"width 1s ease",borderRadius:2}}/></div>
        <div style={{textAlign:"center"}}><div style={{fontSize:22}}>🌙</div><div style={{fontSize:11,color:"#666"}}>Maghrib</div><div style={{fontSize:13,fontWeight:700}}>{toHHMM(prayerMaghrib)}</div></div>
      </div>
      {/* auto-fast toggle */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderTop:"1px solid rgba(167,139,250,0.15)",paddingTop:10}}>
        <div><div style={{fontSize:13,fontWeight:600,color:"#ddd"}}>Auto-Fast</div><div style={{fontSize:11,color:"#666",marginTop:1}}>Start at Fajr, end at Maghrib</div></div>
        <Tog on={autoFast} set={setAutoFast}/>
      </div>
    </div>}

    {/* protocol selector */}
    {!isR&&<div style={{marginBottom:16}}><button onClick={()=>setShowP(!showP)} style={{width:"100%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.09)",borderRadius:14,padding:"13px 16px",color:"#fff",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",fontSize:14}}><span style={{color:"#888"}}>Protocol</span><span style={{fontWeight:700,flex:1,textAlign:"center"}}>{preset.label} — {preset.fast}h fast</span><span style={{color:"#555",fontSize:12}}>{showP?"▲":"▼"}</span></button>{showP&&<div style={{background:"#111118",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,marginTop:4,overflow:"hidden"}}>{IF_PRESETS.map(p=><button key={p.label} onClick={()=>{setPreset(p);setShowP(false);}} style={{width:"100%",padding:"13px 18px",background:"none",border:"none",borderBottom:"1px solid rgba(255,255,255,0.05)",color:preset.label===p.label?"#60A5FA":"#ccc",textAlign:"left",cursor:"pointer",fontSize:14,fontWeight:preset.label===p.label?700:400}}>{p.label} · {p.fast}h fast / {p.eat}h eat {preset.label===p.label?"✓":""}</button>)}</div>}</div>}
...
function Tog({on,set}){return(<div onClick={()=>set(!on)} style={{width:44,height:26,borderRadius:13,cursor:"pointer",background:on?"#A78BFA":"#2a2a3a",transition:"background 0.2s",position:"relative",flexShrink:0}}><div style={{position:"absolute",top:3,left:on?21:3,width:20,height:20,borderRadius:10,background:"#fff",transition:"left 0.2s",boxShadow:"0 1px 4px rgba(0,0,0,0.4)"}}/></div>);}
...
function History({history}){
  ...
  return(<div style={{padding:"20px 24px 0"}}>
    <h2 style={{margin:"0 0 4px",fontSize:22,fontWeight:800}}>Fast History</h2><p style={{margin:"0 0 18px",fontSize:13,color:"#555"}}>Your fasting record</p>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:10}}>{history.map(h=>{const c=h.type==="Health"?"#60A5FA":"#A78BFA";return(<div key={h.id} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:14,padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}><div style={{width:36,height:36,borderRadius:10,background:`${c}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{h.type==="Health"?"💪":"🌙"}</div><div style={{flex:1}}><div style={{fontSize:14,fontWeight:700}}>{h.date}</div><div style={{fontSize:12,color:"#555",marginTop:2}}>{h.done?`${h.hours}h completed`:"Skipped"}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:12,fontWeight:600,color:c,background:`${c}15`,borderRadius:8,padding:"3px 9px"}}>{h.type}</div><div style={{fontSize:11,color:h.done?"#34D399":"#EF4444",marginTop:4,fontWeight:600}}>{h.done?"✓ Done":"✗ Skipped"}</div></div></div>);})}</div>
  </div>);}
...
function Settings({mode,setMode,preset,setPreset,autoFast,setAutoFast,switchMode,fetchPrayerTimes,prayer}){
  ...
  return(<div style={{padding:"20px 24px 0"}}>
    <h2 style={{margin:"0 0 4px",fontSize:22,fontWeight:800}}>Settings</h2><p style={{margin:"0 0 20px",fontSize:13,color:"#555"}}>Customize your experience</p>
    <Sec title="Fasting Mode">
      <Row label="💪  Health & Wellness" right={<Radio checked={mode===MODES.HEALTH} onClick={()=>setMode(MODES.HEALTH)}/>}/>
      <Row label="🌙  Religious (Ramadan)" right={<Radio checked={mode===MODES.RELIGIOUS} onClick={()=>setMode(MODES.RELIGIOUS)}/>}/>
      <Row label="⇄  Switch Mode (Onboard)" right={<button onClick={switchMode} style={{background:"rgba(96,165,250,0.15)",border:"none",borderRadius:10,padding:"5px 12px",color:"#60A5FA",cursor:"pointer",fontSize:12,fontWeight:700}}>Go</button>}/>
    </Sec>
    {mode===MODES.RELIGIOUS&&<Sec title="Ramadan Settings">
      ...
      <Row label="⏰  Auto-Fast (Fajr→Maghrib)" right={<Tog on={autoFast} set={setAutoFast}/>}/>
    </Sec>}
    ...
    <Sec title="Preferences"><Row label="📔  Notifications" right={<Tog on={n} set={sN}/>}/><Row label="❤️  Apple Health" right={<Tog on={ha} set={sHa}/>}/><Row label="🌙  Dark Mode" right={<Tog on={d} set={sD}/>}/></Sec>
    <Sec title="Account"><Row label="⭐  Upgrade to Pro" right={<span style={{fontSize:12,color:"#FBBF24",fontWeight:700}}>$2.99/mo →</span>}/><Row label="📦  Export Data" right={<span style={{fontSize:12,color:"#555"}}>›</span>}/><Row label="ℹ️  About" right={<span style={{fontSize:12,color:"#555"}}>v1.0.0</span>}/></Sec>
  </div>);}
...
function Profile({history}){
  ...
  return(<div style={{padding:"20px 24px 0"}}>
    <h2 style={{margin:"0 0 4px",fontSize:22,fontWeight:800}}>Profile</h2><p style={{margin:"0 0 20px",fontSize:13,color:"#555"}}>Your fasting journey</p>
    <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:24,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:18,padding:"16px 18px"}}><div style={{width:56,height:56,borderRadius:28,background:"linear-gradient(135deg,#60A5FA,#A78BFA)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24}}>⏱</div><div><div style={{fontSize:18,fontWeight:700}}>Ahmad</div><div style={{fontSize:13,color:"#555",marginTop:2}}>FastTracker Pro</div></div><div style={{marginLeft:"auto",background:"rgba(251,191,36,0.15)",borderRadius:10,padding:"4px 10px",fontSize:12,color:"#FBBF24",fontWeight:700}}>PRO ⭐</div></div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>{[{l:"Fasts Done",v:done.length,e:"🏆"},{l:"Total Hours",v:`${total.toFixed(0)}h`,e:"⏱"},{l:"Streak",v:`${done.length}d`,e:"🔥"},{l:"Best Fast",v:`${Math.max(...done.map(h=>h.hours),0)}h`,e:"⭐"}].map(s=><div key={s.l} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:14,padding:"16px",display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:22}}>{s.e}</span><div><div style={{fontSize:20,fontWeight:800}}>{s.v}</div><div style={{fontSize:11,color:"#555",marginTop:1}}>{s.l}</div></div></div>)}</div>
    <Sec title="Achievements">{[{l:"First Fast",done:true,e:"🌱"},{l:"3-Day Streak",done:true,e:"🔥"},{l:"7-Day Streak",done:false,e:"🏅"},{l:"24-Hour Fast",done:false,e:"⚡"}].map(a=><Row key={a.l} label={<span style={{opacity:a.done?1:0.4}}>{a.e}  {a.l}</span>} right={<span>{a.done?"✅":"🔒"}</span>}/>)}</Sec>
  </div>);}

function TabBar({tab,setTab}){return(<div style={{position:"absolute",bottom:0,left:0,right:0,background:"rgba(9,9,15,0.95)",backdropFilter:"blur(30px)",borderTop:"1px solid rgba(255,255,255,0.06)",display:"flex",justifyContent:"space-around",padding:"10px 0 24px"}}>{[{id:TABS.HOME,e:"🏠",l:"Home"},{id:TABS.HISTORY,e:"📊",l:"History"},{id:TABS.SETTINGS,e:"⚙️",l:"Settings"},{id:TABS.PROFILE,e:"👤",l:"Profile"}].map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{background:"none",border:"none",color:tab===t.id?"#60A5FA":"#444",display:"flex",flexDirection:"column",alignItems:"center",gap:3,fontSize:10,fontWeight:600,cursor:"pointer",padding:"4px 16px",transition:"color 0.2s"}}><span style={{fontSize:20}}>{t.e}</span>{t.l}</button>)}</div>);}