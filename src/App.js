...
const BANNER_ID="ca-app-pub-7547768976377458/3333907248";
const INTERSTITIAL_ID="ca-app-pub-7547768976377458/2547823624";
...
function Home({mode,fasting,elapsed,remaining,progress,preset,setPreset,stage,ringColor,R,CIR,drawn,startFast,endFast,prayer,prayerFajr,prayerMaghrib,fmt,fmtS,toHHMM,switchMode,autoFast,setAutoFast,fetchPrayerTimes,isPro}){
  ...
  return(<div style={{padding:"18px 24px 0"}}>
    ...
    {isR&&<div style={{background:"rgba(167,139,250,0.08)",border:"1px solid rgba(167,139,250,0.2)",borderRadius:14,padding:"12px 16px",marginBottom:12}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        ...
        <div style={{textAlign:"center"}}><div style={{fontSize:13,color:"#A78BFA",fontWeight:700}}>Maghrib</div><div style={{fontSize:16,fontWeight:800}}>{toHHMM(prayerMaghrib)}</div></div>
      </div>
...
function History({history}){
  ...
  return(<div style={{padding:"20px 24px 0"}}><h2 style={{margin:"0 0 4px",fontSize:22,fontWeight:800}}>Fast History</h2><p style={{margin:"0 0 18px",fontSize:13,color:"#555"}}>Your fasting record</p>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:10}}>{history.map(h=>{const c=h.type==="Health"?"#60A5FA":"#A78BFA";return(<div key={h.id} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:14,padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}><div style={{width:36,height:36,borderRadius:10,background:c+"18",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:800,color:c,flexShrink:0}}>{h.type==="Health"?"H":"R"}</div><div style={{flex:1}}><div style={{fontSize:14,fontWeight:700}}>{h.date}</div><div style={{fontSize:12,color:"#555",marginTop:2}}>{h.done?h.hours+"h completed":"Skipped"}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:12,fontWeight:600,color:c,background:c+"15",borderRadius:8,padding:"3px 9px"}}>{h.type}</div><div style={{fontSize:11,color:h.done?"#34D399":"#EF4444",marginTop:4,fontWeight:600}}>{h.done?"Done":"Skipped"}</div></div></div>);})}</div>
  </div>);}
function Settings({mode,setMode,preset,setPreset,autoFast,setAutoFast,switchMode,fetchPrayerTimes,prayer,isPro,setIsPro}){
  ...
  return(<div style={{padding:"20px 24px 0"}}><h2 style={{margin:"0 0 4px",fontSize:22,fontWeight:800}}>Settings</h2><p style={{margin:"0 0 20px",fontSize:13,color:"#555"}}>Customize your experience</p>
    ...
    <Sec title="Account">
      <Row label="Pro Status" right={<Tog on={isPro} set={setIsPro}/>}/>
      <Row label="Upgrade to Pro" right={<span style={{fontSize:12,color:"#FBBF24",fontWeight:700}}>$2.99/mo</span>}/>
      <Row label="About" right={<span style={{fontSize:12,color:"#555"}}>v1.0.0</span>}/>
    </Sec>
  </div>);}
